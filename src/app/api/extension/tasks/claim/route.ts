...
import { claimNextTask, HEARTBEAT_MS } from "@/lib/browserTasks";
...
export async function POST(req: Request) {
  ...
  const task = await claimNextTask(auth.userId);
  if (!task) return NextResponse.json({ task: null, reason: "no_work" });
  ...
  return NextResponse.json({
    ...
    // Facts only, and only this user's own. Anything not here is a question the
    // extension must stop and ask about rather than guess at.
    profile: {
      fullName: user.name ?? "",
      email: user.email,
      phone: p.phone ?? "",
