...
WHERE IT SENDS BY ITSELF: many listings on the job boards are cross-posts, and the real application lives on the company's OWN Google Form or another supported employer intake. The student has no account on a company's form, so there is nothing of theirs at risk and the agent fills it in and sends it — no action needed from them. With explicit consent and a connected account, the staged beta can also submit on Internshala. Those rows are labelled "Agent sent" on the dashboard.
WHERE THE STUDENT STILL PRESSES SUBMIT (give these concrete reasons, never just the words "security reasons"): applications that exist only on LinkedIn, Naukri, Unstop or Indeed. (1) Those platforms forbid or aggressively resist automated submission — an account caught doing it can be restricted or banned, and it is the student's account, not ours, that is lost, in the middle of their job search; (2) Grindly does not take custody of those platform passwords; (3) the student keeps the final read of what an employer receives with their name on it. For those, Grindly finds and scores the role, drafts the cover letter and the screening answers, and hands over a ready-to-submit link. The browser extension can auto-FILL supported form fields; the Submit click stays theirs.
